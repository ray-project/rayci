...
if [[ "$HOSTTYPE" == "aarch64" || "$HOSTTYPE" == "arm64" ]]; then
  curl -sSfL "https://golang.org/dl/go1.22.3.linux-arm64.tar.gz" -o "/tmp/golang.tar.gz"
else
  curl -sSfL "https://golang.org/dl/go1.22.3.linux-amd64.tar.gz" -o "/tmp/golang.tar.gz"
fi
