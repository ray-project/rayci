...
func TestDeleteWorkspaceByID(t *testing.T) {
	...
	t.Run("no Content-Type header", func(t *testing.T) {
		...
		if err := api.deleteWorkspaceByID(
			"expwrk_abc",
		); err != nil {
			t.Errorf("unexpected error: %v", err)
		}
...
func TestLaunchTemplateInWorkspace(t *testing.T) {
	t.Run("success", func(t *testing.T) {
		server := httptest.NewServer(http.HandlerFunc(
			func(w http.ResponseWriter, r *http.Request) {
				...
				wantPath := "/api/v2/" +
					"experimental_workspaces/from_template"
				...
				if got := payload["template_id"]; got != "my-tmpl" {
					t.Errorf(
						"template_id = %v, want my-tmpl",
						got,
					)
				}
				if got := payload["cloud_id"]; got != "cld_1" {
					t.Errorf(
						"cloud_id = %v, want cld_1", got,
					)
				}
				if got := payload["project_id"]; got != "prj_1" {
					t.Errorf(
						"project_id = %v, want prj_1", got,
					)
				}
		...
		if got := result["name"]; got != "my-tmpl-20260101" {
			t.Errorf(
				"result[name] = %v, want my-tmpl-20260101",
				got,
			)
		}
	})
	...
	t.Run("non-2xx status", func(t *testing.T) {
		...
		if !strings.Contains(err.Error(), "400") {
			t.Errorf(
				"error = %q, want mention of 400", err,
			)
		}
	})
	...
	t.Run("missing result key", func(t *testing.T) {
		...
		if !strings.Contains(
			err.Error(), "missing 'result' key",
		) {
			t.Errorf(
				"error = %q, want mention of "+
					"missing 'result' key",
				err,
			)
		}
	...
	t.Run("invalid JSON response", func(t *testing.T) {
		...
		if !strings.Contains(
			err.Error(), "failed to parse response body",
		) {
			t.Errorf(
				"error = %q, want mention of "+
