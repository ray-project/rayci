...
func TestBuildDepGraph_Discovery(t *testing.T) {
	...
	// App spec references @base - discovered automatically
	appDir := filepath.Join(tmpDir, "app")
	if err := os.MkdirAll(appDir, 0755); err != nil {
		t.Fatal(err)
	}
	...
	graph, err := buildDepGraph(filepath.Join(appDir, "app.wanda.yaml"), noopLookup)
	if err != nil {
		t.Fatalf("buildDepGraph: %v", err)
	}
...
func TestBuildDepGraph_DiscoveryNotFound(t *testing.T) {
	tmpDir := t.TempDir()
