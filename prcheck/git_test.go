...
func (h *gitTestHelper) writeConfig(maxAdd, maxDel int, ignore []string) string {
	h.t.Helper()
	lines := []string{
		"size:",
		fmt.Sprintf("  max_additions: %d", maxAdd),
		fmt.Sprintf("  max_deletions: %d", maxDel),
	}
	if len(ignore) > 0 {
		lines = append(lines, "  ignore:")
		for _, pfx := range ignore {
			lines = append(lines, "    - "+pfx)
		}
	}
	p := filepath.Join(h.WorkDir, "prcheck-policy.yaml")
