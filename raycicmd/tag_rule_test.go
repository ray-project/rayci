...
func TestTagRuleSetMatchTags_FirstMatchWins(t *testing.T) {
	set := &TagRuleSet{
		tagDefs: map[string]struct{}{"python": {}, "specific": {}},
		rules: []*TagRule{
			{Tags: []string{"specific"}, Lineno: 1, Dirs: []string{"python/specific"}},
	...
	tests := []struct {
		...
		{
			name:            "more specific rule matches first",
			changedFilePath: "python/specific/foo.py",
			want:            []string{"specific"},
			wantBool:        true,
		},
		{
			name:            "general rule matches",
			changedFilePath: "python/other/foo.py",
			want:            []string{"python"},
			wantBool:        true,
		},
		{
...
func TestTagRuleSetMatchTags_NoMatch(t *testing.T) {
	set := &TagRuleSet{
		tagDefs: map[string]struct{}{"python": {}},
		rules: []*TagRule{
